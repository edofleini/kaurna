...
class CLIDispatcher:
    ...
    operation_info={
        'list_secrets':{
            'help':'List the stored secrets.  If you provide --secret-name, only versions of that secret will be shown.  If you provide --secret-name and --secret-version, only the one secret/version will be shown. NOT YET IMPLEMENTED.',
            'initial':'l'
            },
        'rotate_keys':{
        ...
        'erase_secret':{
            'help':'Erase the provided secret from kaurna.  Can be used to delete all versions of a secret, but cannot be used to delete all secrets at once.  For that, use --delete-all-the-things. NOT YET IMPLEMENTED.',
            'initial':'e'
            },
        'deprecate_secrets':{
        ...
        'get_secret':{
            'help':'Download the desired secret.  This will print it to stdout; if you don\'t want it to appear on the screen, you can pipe the output of this command to a file or to a clipboard program like pbcopy or xclip (which one to use varies based on your OS). NOT YET IMPLEMENTED.',
            'initial':'g'
            },
        'erase_all_the_things':{
            'help':'Erase every secret.  Only use this as a last resort.  Even if you pass in --force, this will require a prompt. NOT YET IMPLEMENTED.',
            'initial':None
            }
        }
    ...
    def get_secret(self, **kwargs):
        print 'get_secret called, but not yet implemented'
        exit(1)
    ...
    def get_argument_parser(self):
        ...
        for operation in self.operation_info.keys():
            ...
            if op['initial']:
                operations.add_argument('-{0}'.format(op['initial']), operation_cli, action='store_true', help=op['help'])
            else:
                operations.add_argument(operation_cli, action='store_true', help=op['help'])

        parser.add_argument('--region', default='us-east-1', help='The AWS region to use.')
        parser.add_argument('--secret-name', default=None, help='The name of the secret.  Required for erase-secret, store-secret, and get-secret.  Optional for list-secrets, rotate-keys, deprecate-secrets, activate-secrets, and update-secrets.')
        parser.add_argument('--secret-version', default=None, help='The version of the secret to use.  If this is provided, secret-name must also be provided.  Optional for list-secrets, rotate-keys, store-secret, erase-secrets, deprecate-secrets, activate-secrets, update-secrets, and get-secret.')
        parser.add_argument('--secret', default=None, help='The secret to store.  Currently the only way to enter it is here, but I\'ll add a way to enter it that doesn\'t display it later.  Required for store-secret.')
        parser.add_argument('--authorized-entities', nargs='+', help='The entities that should have permission to access the secret(s).  Optional for update-secrets and store-secret; if not provided the empty list will be used.')
        parser.add_argument('-f', '--force', action='store_true', help='Skip normal confirmation prompts.  Optional for all calls.  Ignored by erase-all-the-things.')
        parser.add_argument('-v', '--verbose', action='store_true', help='Print random usually-useless information.  May or may not print anything depending on whether or not I\ve implemented it yet, as I haven\'t right now.  Optional for all calls.')
